...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const peakIndex = user.peaksAchieved.findIndex(
        (p) => p.peakId.toString() === peakId,
      );

      if (peakIndex === -1) {
        return res
          .status(404)
          .json({ error: "Peak not found in user's achieved list" });
      }
      ...
      res
        .status(200)
        .json({ message: "Image uploaded succesffully and added to user" });
    } catch (error) {
...
router.delete("/delete", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    user.peaksAchieved[peakIndex].imgData = user.peaksAchieved[
      peakIndex
    ].imgData.filter((img) => img.publicId !== publicId);

    if (user.peaksAchieved[peakIndex].imgData.length === initialLength) {
      return res
        .status(404)
        .json({ error: "Image with specified publicId not found" });
    }
