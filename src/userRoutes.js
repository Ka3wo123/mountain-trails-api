...
router.post('/:nick/peaks', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    await User.updateOne({ nick }, { $addToSet: { peaksAchieved: peakToSave } });

    await user.save();

    res.status(201).json({
      message: "Peak added to user's achieved list",
      peaksAchieved: user.peaksAchieved,
    });
  } catch (error) {
...
router.delete('/:nick/peaks', async (req, res) => {
  const { nick } = req.params;
  const { peakId } = req.body;
  ...
  try {
    ...
    if (peakIdx === -1) {
      return res.status(404).json({ error: 'Peak not found in user peaks' });
    }
